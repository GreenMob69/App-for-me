...
const LiveDashboardScreen = () => {
    const {
        ...
        selectedMetric, setSelectedMetric, // Acum va stoca ID-ul din OSCILLOSCOPE_METRICS
    ...
    // =========================================================================
    // CONTROALE NOI DE ZOOM (X și Y)
    // =========================================================================
    const [zoomX, setZoomX] = useState(10); // Spacing-ul dintre puncte (Mărește axa timpului)
    const [zoomYMult, setZoomYMult] = useState(1); // Multiplicator pentru amplitudine (Y)
    ...
    // Extragere din obiectul complex
    const m = liveData.motor || {}; const t = liveData.temperaturi || {}; const a = liveData.aer || {};
    const c = liveData.combustibil || {}; const bat = liveData.baterie || {}; const tr = liveData.transmisie || {}; 
    const meta = liveData.consum_meta || {}; const ecu = liveData.ecu || {};
    ...
    const renderCockpitView = () => (
        <View style={{ flex: 1 }}>
            <View style={styles.filterSection}>
                <Text style={styles.filterTitle}>⚙️ SELECTEAZĂ GRUPUL DE PARAMETRI:</Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll}>
                    {[
                        { id: 'REZUMAT', label: '🌐 REZUMAT ECU' },
                        { id: 'MOTOR', label: '🏎️ MOTOR & DINAMICĂ' },
                        { id: 'AER', label: '🌀 TURBO & AER' }
                    ].map((tab) => (
            ...
            <View style={styles.grid}>
                {activeTab === 'REZUMAT' && [
                    renderCard("VITEZĂ VEHICUL", m.speed || 0, "km/h", "#3fb950", `Treaptă curentă: Tr. ${tr.gear || 1}`, true),
                    renderCard("TURAȚIE MOTOR", m.rpm || 0, "RPM", "#fff", "Plajă optimă Diesel"),
                    renderCard("PRESIUNE TURBO", a.map || 101, "kPa", "#58a6ff", `Boost efectiv: ${a.boost_actual || 0} bar`),
                    renderCard("CONSUM COMBUSTIBIL", c.inst_cons || 0, (m.speed || 0) > 3 ? "L/100km" : "L/h", "#d29922"),
                ]}
                {activeTab === 'MOTOR' && [
                    renderCard("TURAȚIE MOTOR", m.rpm || 0, "RPM", "#fff"),
                    renderCard("SARCINĂ MOTOR", m.load || 0, "%", "#fff"),
                    renderCard("FORȚĂ G", m.accel_g || 0, "G", "#f85149"),
                    renderCard("POZIȚIE CLAPETĂ", m.throttle_pos || 0, "%", "#58a6ff"),
                ]}
                {activeTab === 'AER' && [
                    renderCard("DEBIT MAF", a.maf || 0, "g/s", "#d29922"),
                    renderCard("PRESIUNE MAP", a.map || 101, "kPa", "#58a6ff"),
                ]}
            </View>
        </View>
    );
    ...
    const getChartConfiguration = () => {
        // Găsim configurația metricii selectate
        const config = OSCILLOSCOPE_METRICS.find(m => m.id === selectedMetric) || OSCILLOSCOPE_METRICS[0];
        
        // Extragem toate datele din memorie (Ultimele 10 minute salvate)
    ...
    const renderChartView = () => {
        ...
        return (
            <View style={styles.chartContainer}>
                {/* SELECTORUL DE SENZORI (ACUM AI TOȚI SENZORII DE PE MAȘINĂ!) */}
                ...
                {/* CONTROALELE DE ZOOM (X și Y) TIP HARDWARE */}
                ...
                <View style={styles.chartWrapper}>
                    ...
                    <LineChart
                        ...
                        scrollable={true} showScrollIndicator={true} // ACEASTA PERMITE NAVIGAREA ÎN ISTORIC!
                        pointerConfig={{
