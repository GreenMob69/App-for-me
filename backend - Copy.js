...
client.on('message', (topic, message) => {
    ...
    if (eveniment_motor === "MERS" && calatoriiActive[vin]) {
        ...
        trip.maxSpeed = Math.max(trip.maxSpeed, m.speed || 0);
        trip.sumSpeed += (m.speed || 0);

        trip.maxRPM = Math.max(trip.maxRPM, m.rpm || 0);
        trip.sumRPM += (m.rpm || 0);

        trip.maxCoolant = Math.max(trip.maxCoolant, t.coolant || 0);
        trip.sumCoolant += (t.coolant || 0);

        trip.maxOil = Math.max(trip.maxOil, t.oil || 0);
        trip.sumOil += (t.oil || 0);

        trip.maxBoost = Math.max(trip.maxBoost, a.boost_actual || 0);
        trip.sumBoost += (a.boost_actual || 0);

        trip.maxMAF = Math.max(trip.maxMAF, a.maf || 0);
        trip.sumMAF += (a.maf || 0);

        trip.minVoltage = Math.min(trip.minVoltage, bat.ecu_volt || 14);
        trip.maxVoltage = Math.max(trip.maxVoltage, bat.ecu_volt || 14);
